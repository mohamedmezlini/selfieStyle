...
class BlogController extends Controller
{
    ...
    /**
     * @Route("/pages", defaults={"page": 1}, name="gpage_index")
     * @Route("/page/{page}", requirements={"page": "[1-9]\d*"}, name="blog_index_paginated")
     * @Method("GET")
     * @Cache(smaxage="10")
     */
    ...
    {
        $gpages = $this->getDoctrine()->getRepository(GPage::class)->findLatest($page);

        return $this->render('blog/page.html.twig', ['gpages' => $gpages ] );
    }
...
//*****************************************************
    ...
    /**
     * @Route("/gpages/{slug}", name="blog_gpage")
     * @Method({"GET","POST"})
     *
     * NOTE: The $gpage controller argument is automatically injected by Symfony
     * after performing a database query looking for a Post with the 'slug'
     * value given in the route.
     * See http://symfony.com/doc/current/bundles/SensioFrameworkExtraBundle/annotations/converters.html
     */
    public function gpageShowAction(Request $request, GPage $gpage)
    {
        // Symfony provides a function called 'dump()' which is an improved version
        // of the 'var_dump()' function. It's useful to quickly debug the contents
        // of any variable, but it's not available in the 'prod' environment to
       ...
       $gpages = $entityManager->getRepository(GPage::class)->findBy([], ['publishedAt' => 'DESC']);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('blog_gpage', ['slug' => $gpage->getSlug() ]);
       ...
       /* return $this->render('admin/page/index.html.twig', [
        ...
        if ('dev' === $this->getParameter('kernel.environment')) {
            dump($gpage, $this->get('security.token_storage')->getToken()->getUser(), new \DateTime());
        }

        return $this->render('blog/gpage_show.html.twig', ['gpage' => $gpage,
            'gpages' => $gpages, 
            'posts' => $posts, 
            'events' => $events,
            'post' => $post,
...
//*****************************************************
    ...
    /**
     * @Route("/events/{slug}", name="blog_event")
     * @Method({"GET","POST"})
     *
     * NOTE: The $event controller argument is automatically injected by Symfony
     * after performing a database query looking for a Post with the 'slug'
     * value given in the route.
     * See http://symfony.com/doc/current/bundles/SensioFrameworkExtraBundle/annotations/converters.html
     */
    public function eventShowAction(Request $request, Event $event)
    {
        // Symfony provides a function called 'dump()' which is an improved version
        // of the 'var_dump()' function. It's useful to quickly debug the contents
        // of any variable, but it's not available in the 'prod' environment to
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('blog_event', ['slug' => $event->getSlug() ]);
       ...
       /* return $this->render('admin/page/index.html.twig', [
        ...
        if ('dev' === $this->getParameter('kernel.environment')) {
            dump($event, $this->get('security.token_storage')->getToken()->getUser(), new \DateTime());
        }

        return $this->render('blog/event_show.html.twig', ['event' => $event,
            'events' => $events, 
            'posts' => $posts,
            'gpages' => $gpages,
            'post' => $post,
    ...
    public function commentNewAction(Request $request, Post $post)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->forward('AppBundle:Admin\Page:indexActionPage',[]);
        }
